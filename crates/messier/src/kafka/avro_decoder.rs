...
pub fn init_avro_decoder<'a, 'b>(
    config: &'a KafkaConfiguration,
) -> Result<AvroDecoder<'b>, Error> {
    let sr_settings = resolve_sr_settings(config)?;
    Ok(AvroDecoder::new(sr_settings))
}

pub fn resolve_sr_settings(config: &KafkaConfiguration) -> Result<SrSettings, Error> {
...
impl<'a> AvroRecordDecoder<'a> {
    pub fn new<'b>(config: &'b KafkaConfiguration) -> Result<AvroRecordDecoder<'a>, Error> {
